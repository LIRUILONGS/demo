package com.liruilong.demo.filter;


import com.liruilong.demo.config.MyAuthenticationFailureHandler;
import com.liruilong.demo.config.ValidateCodeException;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, AccessDeniedException,IOException {
        ...
        if (StringUtils.equals("/doLogin", request.getRequestURI())
            ...
            String code = (String) request.getSession().getAttribute("code");
                logger.warn("开始校验验证码，生成的验证码为：" + code + " ，输入的验证码为：" + requestCaptcha+"       SessionId： "+request.getSession().getId());
